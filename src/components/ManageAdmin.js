...
export default function ManageResidents() {
    ...
    //Operation to display all the residents information
     const showAllResident = () => {
        ...
        const url = new URL("http://localhost:80/siremar_backend/getinspectors.php"),
       // const url = new URL("http://data.rxp3138.uta.cloud/siremar_backend/getinspectors.php"),
          params = {}
        Object.keys(params).forEach(key => url.searchParams.append(key, params[key]))
        return fetch(url)
          .then((res) => res.json())
   ...
   const deleteResident = (id) => {
                ...
                const param={id}
                const url = new URL("http://localhost:80/siremar_backend/removeinspector.php"),
                //const url = new URL("http://data.rxp3138.uta.cloud/siremar_backend/removeinspector.php"),
                params = param
            Object.keys(params).forEach(key => url.searchParams.append(key, params[key]))
            return fetch(url)
                .then((res) => res.json())
                                ...
                                <tr>
                                ...
                                </tr>                                                 
                                    {
                                        ...
                                        <tr key={`${index}-resident`}>
                                            ...
                                            <td>{item.county} </td>
                                            <td>{item.contact} </td>                                          
                                            <td><input type="button" onClick={()=>editInfo(item)} value="Edit"/><input type="button" onClick={()=> deleteResident(item.id)} value="Remove"/>
                                            </td>
